...
$badGuids = Get-Content Upgrade-*-error.log | % {$_ -match 'feature.+\[(?<guid>[0-9a-z]+-[0-9a-z-]+[^\]]+)\]' | % {if ($matches.guid) {$matches.guid} } } | sort -Unique
...
# Bad feature discovery method... find all features with a null feature definition under the site or web object "features" property.
...
# Alternate discovery method... find all features with definitionId contained in the badGuids array (no idea if this works... need to test on a fresh import):
write-host "Finding bad site-scoped features..." -ForegroundColor Cyan
...
foreach ($fea in $badWebFeatures) {
    write-host "    Removing ["$fea.DefinitionId"] from '"$fea.parent.url"'." -ForegroundColor Gray
    Disable-SPFeature -Identity $fea.DefinitionId -url $fea.parent.url -force -confirm:$false
    write-host "    Just in case... Removing ["$fea.DefinitionId"] from '"$fea.parent.site.url"'." -ForegroundColor Gray
    Disable-SPFeature -Identity $fea.DefinitionId -url $fea.parent.site.url -force -confirm:$false
}
#Maybe we should do this at the web application layer as well?  SPWebApplication object have a "features" property as well... 
#   Nope!  Investigation shows that there are no null feature definitions at the WebApp layer (not surprising really, since we did not migrate the webapp itself)

#A different approach would be use use a dictionary of known bad feature IDs.  Loop though the sites matching each feature against the bad feature dictionary:
